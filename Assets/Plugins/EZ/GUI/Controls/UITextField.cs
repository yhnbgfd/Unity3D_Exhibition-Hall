...
*/
	...
	//select area start pos,by Egg
	protected int caretSelectStartPos;
	
	//select area end pos,by Egg
	protected int caretSelectEndPos;
	
	//sign if !multiline ,the control is on foucs , by Egg
	...
	//预处理拖拽
	protected void PreProcessMouseDrag()
	{
		if(caret == null)
			return;
		int index = spriteText.DisplayIndexToPlainIndex(spriteText.GetNearestInsertionPoint(caret.transform.position));
		caretSelectStartPos = index;
	}
	
	//处理拖拽
	protected void ProcessMouseDrag()
	{	
		if(caret == null)
			return;
		int index = spriteText.DisplayIndexToPlainIndex(spriteText.GetNearestInsertionPoint(caret.transform.position));
		caretSelectEndPos = index;
		int si = Mathf.Min(caretSelectStartPos,caretSelectEndPos);
		si = Mathf.Clamp(si,0,Text.Length - 1);
		int count = Mathf.Abs(caretSelectStartPos - caretSelectEndPos);
		if(count <= 0)
			return;
		//char[] s = new char[count + 1];
		//Text.CopyTo(si,s,0,count);
		caretSelectEndPos = 0;
	}
	...
	public override void OnInput(ref POINTER_INFO ptr)
	{
		...
		if (hasFocus)
		{
			...
			//implement the select area,by Egg
			if (ptr.evt == POINTER_INFO.INPUT_EVENT.DRAG && caret != null && allowClickCaretPlacement)
			{
				RaycastHit hit;
				if (collider.Raycast(ptr.ray, out hit, ptr.rayDepth))
				{
	...
	//set acitvie,by Egg
	public void SetControlActive(bool tf)
	{
		isActive = tf;
	}
...
*/
	...
	public void LostFocus()
	{
		...
		//lost focus set active false,by Egg
		isActive = false;
		// Hide our caret
		HideCaret();
	}
	...
	protected virtual void PositionCaret(bool recur)   // by KK,  + virtual. for the KUI inherit..
	{
		if (spriteText == null)
			return;
		if (caret == null)
