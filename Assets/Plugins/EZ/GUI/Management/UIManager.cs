...
public class UIManager : MonoBehaviour
{
    ...
    */
    ...
    private IUIObject m_OverObject = null;
    public IUIObject MouseOveringObject   // 当前鼠标在哪个控件上
    {
        get
        {
            return m_OverObject;
    ...
    private bool m_IsProcessedInput = false; //是否处理过输入了
    public bool IsProcessedInput
    {
        get
        {
            return m_IsProcessedInput;
        }
        set
        {
            m_IsProcessedInput = true;
        }
    }
    ...
    float lastKeyDownTime = 0;	//by Egg
    const float delayKeyPress = 0.1f;	//by Egg
    int curActionID = 0;
    int numTouches;
...
// 				else
    ...
    protected void DispatchHelper(ref POINTER_INFO curPtr, int camIndex)
    {
                    ...
                    if (Physics.Raycast(curPtr.ray, out hit, curPtr.rayDepth, curPtr.layerMask))
                    {
                        ...
                        if (curPtr.targetObj != null)
                        {
                                ...
                                ((curPtr.type == POINTER_INFO.POINTER_TYPE.RAY) == focusWithRay))
                            {
                                ...
                                //set the keyfocus control is active
                                if (focusObj != null && focusObj is IKeyFocusable)
                                    ((IKeyFocusable)focusObj).SetControlActive(true);
                            }
                            break;
                        }
        ...
        if (curPtr.targetObj != null)
        {
            if (curPtr.id >= 0)
                usedPointers[curPtr.id] = true;
            m_IsProcessedInput = true; // Been processed input 
            m_OverObject = curPtr.targetObj;// Mouse over the items
        }
        else // add by zhangzhixiong
        {
            m_IsProcessedInput = false; // By the game logic processing input
            m_OverObject = null;  // Mouse over the items did not
        }
    ...
    protected void PollMouse(ref POINTER_INFO curPtr)
    {
        ...
        {
            curPtr.mouseKey = POINTER_INFO.MOUSE_KEY.LEFT_KEY;
        }
        else if (down == Input.GetMouseButton(1))
        {
            curPtr.mouseKey = POINTER_INFO.MOUSE_KEY.RIGHT_KEY;
        }
    ...
    protected void PollKeyboard()
    {
        ...
        {
            if (Time.time - lastKeyDownTime >= delayKeyPress)
            {
    ...
    {
        if (Input.inputString.Length == 0 && !Input.GetKey(KeyCode.Delete))
            return;
        ...
        foreach (char c in Input.inputString)
        {
            ...
            if (c == '\b')
            {
                insert = insert - 1;// Mathf.Max(0, insert - 1),by Egg
                if (insert >= 0 && insert < sb.Length)
                    sb.Remove(insert, 1);
                continue;
            } // Delete
    ...
    /// keyboard focus.Let the key board control can receive ime input,by Egg
    /// </summary>
