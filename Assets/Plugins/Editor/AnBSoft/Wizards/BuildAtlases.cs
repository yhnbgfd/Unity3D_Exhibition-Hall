...
// 		atlas = (Texture2D) AssetDatabase.LoadAssetAtPath(projectRelativePath, typeof(Texture2D));
		...
		int paddingToSet = padding;
		if (texList.textures.Count == 1) paddingToSet = 0;  // µ¥ÕÅÍ¼Æ¬Ê±padding 0
		
		texList.uvs = atlas.PackTextures((Texture2D[])texList.trimmedTextures.ToArray(typeof(Texture2D)), paddingToSet, maxAtlasSize);
